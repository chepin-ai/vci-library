...
```
        ...
        └───┬───────────────────┬───────────────────────┬─────────────┘
            ▼                   ▼                       ▼
   P-A 私仓 CI 船队      P-B 会话端（在线时）      P-C 复用库集中重服务
   ci-bus/ci-playground  usrm/cfts 激活窗口      ci-playground batch
   （cisbr key 舰队）     （创造性/蒸馏推理）      （LEAN/Kaggle/ATP 等）
            │                   │                       │
...
| 会话静默 + 私仓有渗漏分钟 | P-A 私仓船队（cisbr 矩阵选模型） |
